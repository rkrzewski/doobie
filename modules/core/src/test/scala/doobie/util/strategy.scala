...
class strategyspec extends Specification {
  ...
  "Connection configuration and safety (streaming)" >> {
    ...
    "Connection.autoCommit should be set to false" in {
      val i = new Interp
      sql"select 1".query[Int].stream.transact(xa(i)).compile.toList.unsafeRunSync()
      i.Connection.autoCommit must_== Some(false)
    }
    ...
    "Connection.commit should NOT be called on failure" in {
      val i = new Interp
      sql"abc".query[Int].stream.transact(xa(i)).compile.toList.attempt.unsafeRunSync().toOption must_== None
      i.Connection.commit must_== None
    }
  ...
  "PreparedStatement safety (streaming)" >> {
    ...
    "PreparedStatement.close should be called on failure" in {
      val i = new Interp
      sql"select 'x'".query[Int].stream.transact(xa(i)).compile.toList.attempt.unsafeRunSync().toOption must_== None
      i.PreparedStatement.close must_== Some(())
    }
